...
class TestFetchAgents:
    ...
    def test_Creating_an_interval_task(self):
        self.driver.find_element(
            By.XPATH, "//a[text()='Creating an interval task'][1]").click()
        copy_button = self.driver.find_elements(
            By.XPATH, "//button[@title='Copy code']")
        for i in range(len(copy_button)):
            if i == 2:
    ...
    def test_Getting_uAgent_addresses(self):
        self.driver.find_element(
            By.XPATH, "//a[text()='Getting uAgent addresses 🤖📫'][1]").click()
        copy_button = self.driver.find_elements(
            By.XPATH, "//button[@title='Copy code']")
        for i in range(len(copy_button)):
            if i == 4:
    ...
    def test_using_uAgents_storage_function(self):
        self.driver.find_element(
            By.XPATH, "//a[text()='Using uAgents storage function'][1]").click()
        self.driver.find_element(
    ...
    def test_registering_in_the_Almanac_Contract(self):
        self.driver.find_element(
            By.XPATH, "//a[text()='Registering in the Almanac Contract'][1]").click()
        self.driver.find_element(
            By.XPATH, "//button[@title='Copy code']").click()
        time.sleep(3)
    ...
    def test_communicating_with_other_uAgents1(self):
        self.driver.find_element(
            By.XPATH, "//a[text()='Communicating with other uAgents 📱🤖️'][1]").click()
        time.sleep(2)
        copy_button = self.driver.find_elements(
            By.XPATH, "//button[@title='Copy code']")
        for i in range(len(copy_button)):
    ...
    #         result = subprocess.Popen(
    #             ["python", temp_file_path, "--port", "8001"])
    #         print("Python script output:")
    #         print(result.stdout)
    #         time.sleep(9)
    #         sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
